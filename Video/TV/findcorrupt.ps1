...
function Invoke-SonarrReplaceFromPath {
    ...
    try {
        ...
        # File was already removed from the filesystem. Refresh so Sonarr
        # rescans and marks the episode missing (wanted), re-enable
        # monitoring, then trigger a grab/search.
        $refreshBody = @{
            name     = "RefreshSeries"
            seriesId = $series.id
        } | ConvertTo-Json
