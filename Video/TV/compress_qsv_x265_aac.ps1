...
###############################################################
# FUNCTION: Detect interlaced / progressive / unknown sources
###############################################################
function Get-VideoInterlaceStatus {
    param([string]$Path)

    try {
        $probeJson = ffprobe -v quiet -print_format json -show_streams -show_frames -select_streams v "$Path"
    ...
    if ($stream.field_order -and $stream.field_order -notmatch "progressive") {
        return "interlaced"
    }
...
foreach ($f in $files) {
    ...
    switch ($status) {
        "interlaced" {
            $hb_filter = "--deinterlace=yadif"
            Write-Host "Detected: TRUE INTERLACE → Applying YADIF"
        }
        "progressive" {
            $hb_filter = ""
        ...
        "unknown" {
            $hb_filter = "--detelecine --deinterlace=yadif"
            Write-Host "Detected: UNKNOWN / TELECINE → Applying DETELECINE + YADIF"
        }
    }
    ...
    #####################################################
    # RUN HANDBRAKE DIRECTLY (LIKE OLD SCRIPT)
    #####################################################
    ...
    if ($exit -eq 0) {
        ...
        Write-Host "HandBrake failed with exit code $exit"
        if (Test-Path $tmpfile) { Remove-Item $tmpfile -Force }
        continue
    }
}
