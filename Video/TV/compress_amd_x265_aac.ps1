<#
.SYNOPSIS
    Batch compresses TV show episode files using AMD GPU hardware acceleration.

.DESCRIPTION
    Recursively scans the current directory for MKV, MP4, and TS files 1 GB or larger and
    re-encodes files that are not already HEVC+AAC or exceed 2.5 Mbps video bitrate.
    Encoding uses AMD hevc_amf via dxva2 hardware acceleration. Interlace detection is
    performed with ffprobe/idet. The original file is replaced only when the new file is
    smaller. Up to $MaxJobs parallel encoding jobs run at once.
