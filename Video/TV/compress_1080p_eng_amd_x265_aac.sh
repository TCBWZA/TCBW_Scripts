...
#####################################################
    ...
    # Keep only eng/und/unk audio when present.  If no such audio exists,
    # keep ALL audio so foreign-only content is not silenced.
    has_eng_audio=false
    ...
    # Track change detection: when eng/und/unk audio exists, non-matching
    # audio tracks get dropped -- that is a format change, not a skip.
