...
while read -r f; do
    ...
    file_skip_file="${dir}/.skip_${basename}"
    ...
    if [[ -f "$file_skip_file" ]]; then
        echo "Skipping $f -- file marked with .skip_${basename}"
        continue
    fi
    ...
    (
        ...
        if [[ $? -eq 0 ]]; then
            ...
            if (( new_size < orig_size )); then
                ...
                echo "Skipped: new file not smaller ($(( orig_size / 1024 / 1024 ))MB → $(( new_size / 1024 / 1024 ))MB) - creating .skip_${basename}"
