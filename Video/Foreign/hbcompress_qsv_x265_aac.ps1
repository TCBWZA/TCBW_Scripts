<#
.SYNOPSIS
    Transcodes foreign video files to HEVC using HandBrake with Intel QSV hardware encoding.

.DESCRIPTION
    Recursively scans the current directory for MKV, MP4, and TS files over 1 GB and
    transcodes them to HEVC (H.265) video using HandBrakeCLI and the Intel Quick Sync
    Video (qsv_h265) hardware encoder. All audio tracks and subtitles are copied without
    modification or language filtering.
...
.EXAMPLE
    PS> .\hbcompress_qsv_x265_aac.ps1

.EXAMPLE
    PS> .\hbcompress_qsv_x265_aac.ps1 -Debug
